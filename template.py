...
def make_prices_json(root: Path) -> None:
    ...
    path = root / "src" / "llm_watchdog" / "pricing" / "prices.json"
    path.parent.mkdir(parents=True, exist_ok=True)
...
def make_config(root: Path) -> None:
    write(root / "src" / "llm_watchdog" / "config.py", """
        \"\"\"
...
def make_exceptions(root: Path) -> None:
    write(root / "src" / "llm_watchdog" / "exceptions.py", """
        \"\"\"
...
def make_models(root: Path) -> None:
    write(root / "src" / "llm_watchdog" / "storage" / "models.py", """
        \"\"\"
...
def make_db(root: Path) -> None:
    write(root / "src" / "llm_watchdog" / "storage" / "db.py", """
        \"\"\"
...
def make_providers(root: Path) -> None:
    write(root / "src" / "llm_watchdog" / "providers" / "base.py", """
        \"\"\"
    ...
    write(root / "src" / "llm_watchdog" / "providers" / "openai.py", """
        \"\"\"
    ...
    write(root / "src" / "llm_watchdog" / "providers" / "anthropic.py", """
        \"\"\"
...
def make_pricing(root: Path) -> None:
    write(root / "src" / "llm_watchdog" / "pricing" / "table.py", """
        \"\"\"
...
def make_core(root: Path) -> None:
    for name, day in [
        ...
        write(root / "src" / "llm_watchdog" / "core" / name, f"""
            \"\"\"
            {name} — PLACEHOLDER
...
def make_cli(root: Path) -> None:
    write(root / "src" / "llm_watchdog" / "cli" / "__main__.py", """
        \"\"\"
    ...
    write(root / "src" / "llm_watchdog" / "cli" / "commands.py", """
        \"\"\"
...
def make_init(root: Path) -> None:
    write(root / "src" / "llm_watchdog" / "__init__.py", """
        \"\"\"
        llm-watchdog
        ============
...
def make_tests(root: Path) -> None:
    ...
    write(root / "tests" / "unit" / "test_placeholder.py", """
        ...
        def test_package_importable():
            ...
            import llm_watchdog
            assert llm_watchdog.__version__ == "0.1.0"
        ...
        def test_exceptions_importable():
            ...
            from llm_watchdog.exceptions import (
                WatchdogError,
                BudgetExceeded,
                ProviderNotDetected,
...
def scaffold(base_path: str = ".") -> None:
    ...
    for p in ["__init__.py"]:
        touch(root / "src" / "llm_watchdog" / "storage" / p)

    print("\n── Providers ────────────────────────────────────")
    make_providers(root)
    for p in ["__init__.py"]:
        touch(root / "src" / "llm_watchdog" / "providers" / p)

    print("\n── Pricing ──────────────────────────────────────")
    make_pricing(root)
    make_prices_json(root)
    touch(root / "src" / "llm_watchdog" / "pricing" / "__init__.py")

    print("\n── Core (decorators, budget, interceptor) ───────")
    make_core(root)
    touch(root / "src" / "llm_watchdog" / "core" / "__init__.py")
